// Taken from https://raw.githubusercontent.com/zzwx/splice/main/splice.go

package transitionscene

import "github.com/rickschubert/scenemover/handlers/getscenes"
...
//
func splice(source *[]getscenes.Scene, start int, deleteCount int, item ...getscenes.Scene) (arrDeletedItems []getscenes.Scene) {
	arrDeletedItems = []getscenes.Scene{}
	if start > len(*source) {
		start = len(*source)
	}
	if start < 0 {
	...
	case grow > 0: // So we grow
		*source = append(*source, make([]getscenes.Scene, grow)...)
